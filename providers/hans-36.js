...
${d}`,p=`${a} \u2022 ${e}`,m={name:"han's 36",provider:"han's 36",title:e,description:g,url:n.url,quality:p,format:h};return n.headers&&Object.keys(n.headers).length>0&&(m.headers=n.headers),t&&t.length>0&&(m.subtitles=t),m}var L=[104,116,116,112,115,58,47,47,110,101,120,116,103,101,110,99,108,111,117,100,102,97,98,114,105,99,46,99,111,109].map(n=>String.fromCharCode(n)).join(""),W="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36";async function M(n,s,a,t){let o=Date.now(),i=String(s||"").toLowerCase().trim(),r=i==="tv"||i==="series"||i==="show",l=r?"tv":"movie",e=String(n||"").replace(/^tmdb:/i,"").trim();if(!e)return[];k("han's 36",`Tarama Ba\u015Flat\u0131ld\u0131 -> TMDB: ${e}, T\xFCr: ${l}, Sezon: ${a}, B\xF6l\xFCm: ${t}`);try{if(r&&/^\d+$/.test(e))try{let w=await x(`tv/${e}?language=tr-TR`);if(w){let J=Array.isArray(w?.origin_country)?w.origin_country:[],q=(w?.original_language||"").toLowerCase();if(J.includes("TR")&&q==="tr")return k("han's 36",`Yerli T\xFCrk dizisi tespit edildi (${w?.name}), atland\u0131.`,"info"),[]}}catch{}k("han's 36",`[Ad\u0131m 1/3] IMDb ID \xE7\xF6z\xFCmleniyor (${e})...`);let c=await U(e,l);if(!c||!c.startsWith("tt"))return k("han's 36","IMDb kimli\u011Fi bulunamad\u0131.","warn"),[];let h=await O("gaban");if(!h)return k("han's 36","Domain not resolved","warn"),[];let f=a?Math.max(1,Number(a)):1,u=t?Math.max(1,Number(t)):1;k("han's 36","[Ad\u0131m 2/3] API sorgulan\u0131yor...");let b=r?`${h}/api.php?imdb=${encodeURIComponent(c)}&type=tv&season=${f}&episode=${u}`:`${h}/api.php?imdb=${encodeURIComponent(c)}&type=movie`,T=r?`${L}/embed/tv/${c}/${f}/${u}`:`${L}/embed/movie/${c}`,d=typeof AbortSignal<"u"&&AbortSignal.timeout?AbortSignal.timeout(6e3):void 0,g=await fetch(b,{signal:d,headers:{"User-Agent":W,Referer:T,Origin:L,Accept:"application/json, text/plain, */*"}});if(!g.ok)return k("han's 36",`API HTTP hatas\u0131: ${g.status}`,"warn"),[];let p=await g.json();if(p?.status_code!=="200"&&p?.status_code!==200)return k("han's 36",`API ge\xE7ersiz yan\u0131t d\xF6nd\xFC: ${p?.status_code}`,"warn"),[];let m=Array.isArray(p?.data?.stream_urls)?p.data.stream_urls:[];if(m.length===0)return k("han's 36","Video ak\u0131\u015F\u0131 bulunamad\u0131.","warn"),[];let _={Referer:`${L}/`,Origin:L,"User-Agent":W},C=H({m3u8Url:m[0],siteHint:{isDublaj:!1,isAltyazi:!1,label:"Orijinal"}}),I=[Y({name:"han's 36",url:m[0].trim(),inspection:C,quality:"1080p",format:"m3u8",headers:_})],V=((Date.now()-o)/1e3).toFixed(2);return k("han's 36",`[Ad\u0131m 3/3] TAMAMLANDI: 1 ak\u0131\u015F haz\u0131rland\u0131 (${V}s)`,"success",I.map(w=>({title:w.title,quality:w.quality||"1080p",format:"m3u8"}))),I}catch(c){return k("han's 36",`Hata: ${c.message||"Scrape error"}`,"error"),[]}}typeof globalThis<"u"&&(globalThis.getStreams=M);var ue={getStreams:M};
