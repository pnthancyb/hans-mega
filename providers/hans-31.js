...
${C}`,h=`${i} \u2022 ${a}`,d={name:"han's 31",provider:"han's 31",title:a,description:u,url:e.url,quality:h,format:S};return e.headers&&Object.keys(e.headers).length>0&&(d.headers=e.headers),n&&n.length>0&&(d.subtitles=n),d}var R="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/130.0.0.0 Safari/537.36",re='"Chromium";v="130", "Google Chrome";v="130", "Not-A.Brand";v="99"';async function G(e,t,i,n){let o=Date.now();k("han's 31",`Tarama Ba\u015Flat\u0131ld\u0131 -> TMDB: ${e}, T\xFCr: ${t}, Sezon: ${i}, B\xF6l\xFCm: ${n}`);try{let s=String(e||"").trim(),r=String(t||"").toLowerCase().trim(),g=r==="tv"||r==="series",a=g?"tv":"movie",A=i!=null?parseInt(String(i),10):1,S=n!=null?parseInt(String(n),10):1;if(!s)return[];let c=await $("rayleigh"),l="";g?l=`${c}/api/tv/${s}/${A}/${S}`:l=`${c}/api/movie/${s}`,k("han's 31","[Ad\u0131m 1/3] Metadata API sorgulan\u0131yor...");let y={"User-Agent":R,"sec-ch-ua":re,"sec-ch-ua-mobile":"?0","sec-ch-ua-platform":'"Windows"',Accept:"application/json, text/javascript, */*; q=0.01","X-Requested-With":"XMLHttpRequest",Referer:c+"/"},p=await fetch(l,{headers:y,signal:AbortSignal.timeout?AbortSignal.timeout(3500):void 0});if(!p.ok)return k("han's 31",`API yan\u0131t vermedi: HTTP ${p.status}`,"warn"),[];let u=(await p.json())?.src;if(!u)return k("han's 31","API yan\u0131t\u0131nda embed ba\u011Flant\u0131s\u0131 bulunamad\u0131.","warn"),[];let h=`${c}${u}`;k("han's 31","[Ad\u0131m 2/3] Oynat\u0131c\u0131 embed sayfas\u0131 \xE7\xF6z\xFCmleniyor...");let d=await fetch(h,{headers:{Referer:c+"/","User-Agent":R},signal:AbortSignal.timeout?AbortSignal.timeout(3500):void 0});if(!d.ok)return k("han's 31",`Embed sayfas\u0131 y\xFCklenemedi: HTTP ${d.status}`,"warn"),[];let m=await d.text(),T=null,f=m.match(/window\.video\s*=\s*\{[^}]*id:\s*'([^']+)'/);if(f)T=f[1];else{let w=u.match(/\/embed\/([^?\/#]+)/);w&&(T=w[1])}if(!T)return k("han's 31","Video ID \xE7\xF6z\xFCmlenemedi.","warn"),[];let D=m.match(/window\.masterPlaylist\s*=\s*\{[\s\S]*?'token':\s*'([^']+)'/),x=m.match(/window\.masterPlaylist\s*=\s*\{[\s\S]*?'expires':\s*'([^']+)'/),P=m.match(/window\.masterPlaylist\s*=\s*\{[\s\S]*?url:\s*'([^']+)'/),I=D?D[1]:null,N=x?x[1]:null,F=P?P[1]:`${c}/playlist/${T}?b=1`;if(!I||!N)return k("han's 31","Token veya s\xFCre a\u015F\u0131m\u0131 bilgisi bulunamad\u0131.","warn"),[];let H=m.includes("window.canPlayFHD = true"),K=F.includes("?")?"&":"?",j=`${F}${K}token=${I}&expires=${N}`;H&&(j+="&h=1");let U=!1;try{let w=await fetch(j,{headers:{"User-Agent":R,Referer:h,Origin:c},signal:AbortSignal.timeout?AbortSignal.timeout(2500):void 0});if(w.ok){let z=(await w.text()).toLowerCase();U=z.includes("type=subtitles")&&(z.includes('name="turkish"')||z.includes('name="t\xFCrk\xE7e"')||z.includes('language="tr"')||z.includes('language="tur"')||z.includes('rendition="tr')||z.includes('rendition="tur'))}}catch{}let E=[B({name:"han's 31",url:j,languageTitle:U?"Altyaz\u0131l\u0131":"Yabanc\u0131",quality:"1080p",format:"m3u8",headers:{"User-Agent":R,Referer:h,Origin:c}})],Y=((Date.now()-o)/1e3).toFixed(2);return k("han's 31",`[Ad\u0131m 3/3] TAMAMLANDI: 1 stream haz\u0131rland\u0131 (${Y}s)`,"success",E.map(w=>({title:w.title,quality:"1080p",format:"m3u8"}))),E}catch(s){return k("han's 31",`Kritik Hata: ${s.message}`,"error"),[]}}typeof globalThis<"u"&&(globalThis.getStreams=G);
