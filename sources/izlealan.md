# izlealan kaynak eşlemesi

- Kaynak manifest: https://nuvio.ayruki.workers.dev/
- Kaynak sürüm: 1.14.561
- Aktif kaynak sayısı: 36
- Nuvio manifestindeki provider adları: `han's 1` – `han's 36`

Yerel provider dosyaları kaynak JS'lerinin Han markalı aynalarıdır. Kaynak provider isimleri kullanıcıya görünen manifest metadata'sına taşınmaz.

| Han numarası | Kaynak ID | Kaynak dosyası |
| ---: | --- | --- |
| 1 | `imu` | `providers/imu.js` |
| 2 | `joyboy` | `providers/joyboy.js` |
| 3 | `enel` | `providers/enel.js` |
| 4 | `crocodile` | `providers/crocodile.js` |
| 5 | `xebec` | `providers/xebec.js` |
| 6 | `kidd` | `providers/kidd.js` |
| 7 | `shiki` | `providers/shiki.js` |
| 8 | `emeth` | `providers/emeth.js` |
| 9 | `saul` | `providers/saul.js` |
| 10 | `garp` | `providers/garp.js` |
| 11 | `ryuma` | `providers/ryuma.js` |
| 12 | `rouge` | `providers/rouge.js` |
| 13 | `kalgara` | `providers/kalgara.js` |
| 14 | `sakazuki` | `providers/sakazuki.js` |
| 15 | `shanks` | `providers/shanks.js` |
| 16 | `kizaru` | `providers/kizaru.js` |
| 17 | `smoker` | `providers/smoker.js` |
| 18 | `zunesha` | `providers/zunesha.js` |
| 19 | `oden` | `providers/oden.js` |
| 20 | `fujitora` | `providers/fujitora.js` |
| 21 | `lili` | `providers/lili.js` |
| 22 | `sabo` | `providers/sabo.js` |
| 23 | `vegapunk` | `providers/vegapunk.js` |
| 24 | `mihawk` | `providers/mihawk.js` |
| 25 | `noland` | `providers/noland.js` |
| 26 | `doflamingo` | `providers/doflamingo.js` |
| 27 | `dragon` | `providers/dragon.js` |
| 28 | `teach` | `providers/teach.js` |
| 29 | `roger` | `providers/roger.js` |
| 30 | `kuzan` | `providers/kuzan.js` |
| 31 | `rayleigh` | `providers/rayleigh.js` |
| 32 | `gorosei` | `providers/gorosei.js` |
| 33 | `ace` | `providers/ace.js` |
| 34 | `hiriluk` | `providers/hiriluk.js` |
| 35 | `urouge` | `providers/urouge.js` |
| 36 | `gaban` | `providers/gaban.js` |

Bu dosya `sync-izlealan.mjs` ve GitHub Actions tarafından otomatik güncellenir.
